...
select max(concat(substr(hire_date, 1, 4), '년 ', substr(hire_date, 6, 2), '월 ', substr(hire_date, 9, 2), '일')) '입사일'
from employees
;

select date_format( max(hire_date), '%Y년 %m월 %e일' )  '입사일'
from employees
;
...
select date_format(min(hire_date), '%Y-%m-%d %W') '입사일'
from employees
;
...
select hire_date '입사일'
	 , case when hire_date < '02/12/31' then '창립멤버'
            ...
            when hire_date > '03/12/31' and hire_date < '05/01/01' then '04입사'
